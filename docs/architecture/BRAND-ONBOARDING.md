...
Status: **Canonical real-brand onboarding contract; Gate 9 Trial pilot acceptance closed within its nonproduction scope**

This document describes the accepted onboarding model after the reusable-core, synthetic-edge, fixed-input, capability/navigation, provider-isolation, bounded Categories-discovery, bounded Home-content, and provisioning/onboarding gates. Gates 1–7 established the application-module architecture and the current Menu/Home contracts. Gate 8 added the strict application/profile registry, deterministic non-secret projections, scoped ignored client configuration, independent provider bindings, explicit enrollment/CI coverage, and a bounded local operator with Inspect, Plan, guarded Apply, Readback, and recovery evidence. At Gate 8 closure, those facts applied to the Gürbakır development/staging profiles and no additional real merchant application yet existed. Gate 9 has since added and accepted the development-only `:trial` real application/store pilot with independent application, Shopify, Customer Account, Firebase and persistence identities. Later exact-main natural-expiry and lifecycle evidence closed A5 within that pilot; Trial is not a production application.

For production onboarding, record merchant-owned Shopify commerce/policy/support/privacy/domain work, app-provider Android engineering/technical response, and a per-brand selection of Play publisher/release, upload-key custody, Firebase administration, domain association coordination and Play declaration submission. The [current operating contract](../operations/PRIVACY-RELEASE-OPERATING-CONTRACT.md) defines these functions. Gürbakır selects project-managed Play/signing/release operation without making that model normative for Trial or future brands.
...
| Account deletion request | If Account is enabled for release, a distinct brand-level HTTPS origin/relative path, signed-out public request resource, merchant intake and accepted provider handoff, plus accountable restricted-case and fulfillment/notice operations; generic Support does not substitute |
...
| Deployment ownership | Accountable merchant/app-provider side for Play publisher and release operation, upload-key custody, Firebase administration, domain/App Links deployment coordination, and Play listing/Data Safety submission; no inferred individual |
