...
This document classifies multi-brand ownership decisions under the architecture described in [Multi-Brand Architecture](MULTI-BRAND-ARCHITECTURE.md). Gates 1–3 established the reusable-core base, second non-production application and application-owned fixed inputs. Gates 4–7 implemented capability/navigation, provider isolation, bounded Shopify Menu and Home-v1 ownership. Gate 8 implemented bounded onboarding/operator contracts. Gate 9 added the independent development-only `:trial` real application/store pilot and bounded Home-v2 media contract without a runtime merchant switch. Gates 4–9 are closed within their recorded scopes; Gate 9's later exact-main acceptance is in its [handoff](../multi-brand/GATE-9-PILOT-ACCEPTANCE-AND-HANDOFF.md). Gürbakır is still the first production-intended validation brand; Trial is not a production application.

## Shopify-aligned by default

Merchant-owned commerce, policy, privacy/content and operational configuration should be sourced from, synchronized with, or kept consistent with Shopify wherever Shopify exposes a suitable authoritative mechanism. Before adding a separate merchant-configurable Android state, inspect current Shopify capabilities, Android/Google Play requirements and this repository's trust boundaries; consult the neutral reference model for behavior only where useful. Prefer one merchant authority. Introduce Android-specific state when platform UX or policy, security, offline/runtime behavior, or an inadequate Shopify mobile contract gives a concrete reason. Re-evaluate the choice when markets, provider capabilities, policies, pixels or SDKs change.

Operational accountability follows the [privacy and release operating contract](../operations/PRIVACY-RELEASE-OPERATING-CONTRACT.md): merchant Shopify/policy/support/privacy work remains merchant-owned; the app provider owns Android engineering and technical remediation; Play publisher, signing, Firebase administration and domain/App Links deployment coordination are selected per brand deployment model. Those organizational choices do not become runtime brand switches or shared-module dependencies.

Shopify alignment does not give remote content native execution or routing authority, expose Admin credentials in a public client, duplicate every Admin field, or make site cookies a native consent preference. Android retains typed/allowlisted routes, finite rendering, installed identity, security and truthful native UX. For example, Shopify owns the deletion request resource and erasure operations while Android owns its trusted route and never infers submission from browser return; Shopify Menu/Home provide bounded content rather than arbitrary components. Shopify's [browser Customer Privacy API](https://shopify.dev/docs/api/customer-privacy) and [mobile Checkout Kit privacy contract](https://shopify.dev/docs/storefronts/mobile/checkout-kit/privacy-compliance) (reviewed 2026-09-26) have different surfaces: a hosted site choice is not automatically available as an Android preference. Investigate the actual privacy model before adding native consent state or sending Storefront `visitorConsent` values.
...
The enabled Account deletion request is a separate typed shared role. Each brand application owns its exact HTTPS merchant resource through its registry identity; the shared screen does not take an arbitrary Shopify URL or infer request submission from browser return. Gürbakır has a dedicated published request page. Trial has no provisioned deletion page, so its development-only adapter omits that action; synthetic has no Account capability. Merchant receipt, verification, retention and remote erasure remain merchant operations. See the [scoped continuation](../phase3/PRE-P3-16-ACCOUNT-DELETION-ROUTE-AND-PROCESS.md).
...
Gate 8 adds bounded registry/projection/operator ownership and fail-closed provider readback without making Android runtime behavior an Admin guarantee. Gate 9 applies that contract to a separate Trial application/store/Firebase identity and adds only the approved finite Home-v2 image/video types, lifecycle budgets and isolated cache/auth/application boundaries. Its protected merge and exact merged-main canonical CI passed; later exact-main natural-expiry and lifecycle evidence closed its defined pilot acceptance. See the [Gate 9 handoff](../multi-brand/GATE-9-PILOT-ACCEPTANCE-AND-HANDOFF.md). This does not prove production signing, Play publication or P3-16 readiness.
