...
Current example: `:app` owns the Gürbakır development/staging IDs, Gürbakır application/theme/icons, `gurbakir.com` App Links and the current Customer Account callback scheme.
...
- approved structured mobile sections/pages through finite typed contracts;
- Product/Collection-specific mobile metadata through typed references.

Current Home and Catalog merchant handles are evidence of content currently embedded in the Gürbakır application, not the target owner for shared merchant hierarchy.
...
| Home/Catalog merchant hierarchy | Merchant-editable content | Shopify bounded content/navigation after its migration gate |
