...
Status: **public route and merchant intake witnessed; one synthetic no-order Shopify erasure request accepted and scheduled for 2026-10-05; provider completion and end-to-end merchant closure remain unverified** (2026-09-25). P-18 remains **PARTIAL — EXTERNAL PROCESS STILL REQUIRED**.
...
The new case is an unmistakably tagged, project-controlled Shopify customer with zero orders. The owner used Shopify Admin to submit **personal-data erasure** for that exact synthetic profile. The Admin banner schedules erasure for **2026-10-05**. A later profile-deletion attempt was refused because the erasure request is pending; read-only Admin GraphQL still finds the zero-order profile and now reports `canDelete=false`. This proves an accepted **pending** provider request, not redaction, profile removal or completion. Shopify's [current data-request guidance](https://help.shopify.com/en/manual/privacy-and-security/privacy/processing-customer-data-requests) says the provider processes a submitted request without a second submission, provides a ten-day cancellation window, distinguishes erasure from profile deletion, and leaves the merchant responsible for companies outside installed apps and channels. Do not cancel or repeat the pending request to test retries.
...
The restricted synthetic ledger is outside Git. A local deterministic duplicate/unknown-result/retry process rehearsal passed nine checks, but live failure handling, complete processor review, final redaction readback, customer completion/partial-retention notice and permanent operator assignment remain open. The owner-reported acknowledgement is not a witnessed sent/delivered message. The synthetic order-bearing fixture was inspected read-only: its one test order makes profile deletion ineligible, and requester control for that historical identity remains unverified. It was not altered. Current operational disposition is **P-18 PARTIAL — EXTERNAL PROCESS STILL REQUIRED**; Shopify's scheduled date is a future readback point, not a completion claim or merchant response SLA.
